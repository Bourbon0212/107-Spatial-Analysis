...
TPE_LI$CENSUS <- as.numeric(as.character(TPE_LI$CENSUS)) #factor > numeric
```
...
ggplot() +
  ...
  scale_fill_gradientn("人口密度(千人/平方公里)", colors = brewer.pal(9, "YlOrRd")) +
...
##Q2-1
```{r}
MRT.near <- gBuffer(MRT, width = 500, byid = T) #buffer500
TPE.MRT <- gIntersection(TPE_LI, MRT.near, byid = T)#做交集
TPE.MRT_name=strsplit(names(TPE.MRT), " ") 
TPE.MRT$TPE_id=as.numeric(unlist(lapply(TPE.MRT_name,function(x) x[1]))) #TPE_id
TPE.MRT$MRT_id=as.numeric(unlist(lapply(TPE.MRT_name,function(x) x[2]))) #MRT_id

TPE.MRT$area=as.vector(poly.areas(TPE.MRT))
TPE.MRT$TPE.area=poly.areas(TPE_LI)[TPE.MRT$TPE_id+1] #TPE_LI從0開始，要+1
